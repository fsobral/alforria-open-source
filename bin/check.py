...
def check_course_sem(cs, S1INI, S2INI, verbosity=True):
    ...
    for c in cs:
        ...
        if (not (c.semestralidade == 1 and c.dini == S1INI)) and \
           ...
           logger.warn("Semestralidade incompatível: {0:12s}\t{1:d}\t{2:10s}".format(c.id(),
                        c.semestralidade, date.isoformat(c.dini)))
...
def check_g(cs, verbosity=True):
    ...
    for c in cs:
        ...
        if c.grupo is None:
            ...
            logger.info("Turma %40s\t%6s\t%d\t%4d sem grupo.", c.nome, c.codigo, c.semestralidade, (17 * c.ch))
