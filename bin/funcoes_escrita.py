...
def gera_declaracao_ch_semestre(listah, s, dirh='/home/fsobral/GoogleDrive/Alforria/2024/horarios'):
    ...
    ch = [((datetime.date.fromisoformat(str(datas[i + 1])) - datetime.date.fromisoformat(str(datas[i]))).days + 1) // 7 for i in range(0, len(datas) - 1)]
    ...
    dfdisc = pd.concat([pd.read_csv('{1:s}/{0:d}/atribuicoes.csv'.format(d, dirh), sep=',', header=None) for d in v]).groupby([1, 2, 3, 6]).agg({4:'first'})

    # Calcula a carga horária total
    
    soma_ch = pd.concat([pd.read_csv('{1:s}/{0:d}/atribuicoes.csv'.format(d, dirh), sep=',', header=None).set_index([1, 2, 3, 6])[5] * c for d, c in zip(v, ch)], axis=1)
    ...
    with open(outfile, "w") as fp:
        ...
        for t in turmas:
            ...
            for h in t.horarios:
                ...
                fp.write("{cod:s},{turma:s},{nome:s},T,99,9,{horario[0]:d},{horario[1]:d},9,CAMPUS,BLOCO,9,{ch:.1f},{cht:.1f},S{semestre:d},{ini:%Y-%m-%d},{fim:%Y-%m-%d},{curso:s}\n".format( cod=t.codigo, turma=t.turma, nome=t.nome, horario=h, ch=t.ch, cht=(t.ch * 17), semestre=t.semestralidade, ini=t.dini, fim=t.dend, curso=t.curso))
    ...
    with open(arquivo,"w") as f:
        ...
        for p in professores:
            ...
            for t in p.turmas_a_lecionar:
                ...
                f.write("{matricula:08d}\t{nome:s}\t{cod:s}\t{turma:s}\t{tnome:s}\t{ch:d}\t{sem:s}\t{curso:s}\n".format(matricula=p.matricula, nome=p.nome(), cod=t.codigo, turma=t.turma, tnome=t.nome, ch=t.carga_horaria(), sem=sem, curso=t.curso))
...
def escreve_disciplinas(professores, turmas, arquivo):
    ...
    with open(arquivo,"w") as f:
        ...
        for t in sorted(turmas, key=lambda x: int(x.codigo)):
            ...
            f.write("{cod:s},{turma:s},{tnome:s},{nome:s},S{sem:s},{anual:s}\n".format(cod=t.codigo, turma=t.turma, tnome=t.nome, nome="" if t.professor is None else t.professor.nome(), sem=str(t.semestralidade), anual=("*" if t.vinculada else "")))
...
def escreve_jl(professores,turmas,grupos,pre_atribuidas,arquivo):
    with open(arquivo, "w") as f:
        ...
        # Supoe que se uma disciplina eh vinculada, seu vinculo eh a 
        # disciplina seguinte
        f.write("vinculadas :: Set{Tuple{String, String}} = Set{Tuple{String,String}}([\n")
        vinc = False
        for t in turmas:
            if vinc:
                f.write(f'"{t.id()}"),\n')
