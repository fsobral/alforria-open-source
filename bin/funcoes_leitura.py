...
def sar292_to_df(sarfile):
        ...
        rx = re.compile(r"^[\s]*[\d]{1,3}[\s]*2024[\s]*DMA[\s]*([\d]{3,5})[\s]*([\d]{1,3})[^\d]+([\d]{1,3})")
        ...
        return pd.DataFrame(entries, columns=['curso', 'codigo', 'turma', 'matriculados', 'professor'])
...
def _csv_to_dataframe(sarfile):
        ...
        df = pd.read_csv(sarfile, header=None, parse_dates=[15, 16], delimiter="\s*,\s*", engine='python', comment='#')
...
def _load_turmas(df_sar, grupos):
        ...
        for t in [t for t in turmas if (t.dend - t.dini).days > 180]:
                ...
                for tt in [tt for tt in turmas if not tt.vinculada]:
                        ...
                        if t.codigo == tt.codigo and t.turma  == tt.turma and t.semestralidade != tt.semestralidade:
...
#----------------------------------------------------------------------------------------------------------------------
def ler_sar(arquivo,grupos,cursos={}): #arquivo: arquivo do SAR, grupos: lista dos objetos grupos
        turmas = []
        anual=False
        invalida = False
        t = None
