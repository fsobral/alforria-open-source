...
function preencheGrupos!(conj::ConjuntosAlforria, sar::ParametrosSAR)
	for t in conj.T
		if !haskey(sar.turma_grupo, t)
			sar.turma_grupo[t] = "__NOGROUP__"
		end
	end
...
function preencheSAR!(sar::ParametrosSAR, conj::ConjuntosAlforria) 
	for t in conj.T
		if !haskey(sar.ch, t)
			sar.ch[t] = sum(1 for s in conj.S, d in conj.D, h in conj.H if (t, s, d, h) in sar.c, init=0)
		end

		if !haskey(sar.ch1, t)
			sar.ch1[t] = sum(Int64[1 for d in conj.D, h in conj.H if (t, conj.S[1], d, h) in sar.c], init=0)
		end

		if !haskey(sar.ch2, t)
			sar.ch2[t] = sum(Int64[1 for d in conj.D, h in conj.H if (t, conj.S[2], d, h) in sar.c], init=0)
		end
	end
end
...
function alforria(
	...
	setFuncaoObjetivo!(alforria_mod, opt.fobj, conj, var)
